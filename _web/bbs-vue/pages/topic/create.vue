        ...
        <div v-if="postForm.type === 0" class="field">
          <div class="control">
            <markdown-editor
              ...
              placeholder="请输入你要发表的内容(上传文件2M内)..."
            />
          </div>
        </div>
  ...
  methods: {
    ...
    async showCaptcha() {
      if (this.config.topicCaptcha) {
        try {
          const ret = await this.$axios.get('/api/captcha/captchaImage', {
            params: {
              captchaId: this.captchaId || '',
            },
          })
          this.captchaId = ret.uuid
          this.captchaUrl = 'data:image/gif;base64,' + ret.img
        } catch (e) {
          this.$message.error(e.message || e)
        }
      }
