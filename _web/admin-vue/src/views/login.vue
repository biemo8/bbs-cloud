    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch("GetAuth", this.loginForm).then((res) => {
            // return new Promise((resolve, reject)=>{
            //   window.location.href = `http://118.190.247.33:80/biemo-auth-app/sso/login?clientId=10001&code=${res.data.code}`;
            //   resolve();
            // }).then(()=>{
